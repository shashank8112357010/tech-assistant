  ...
  const createGroundPlain = useCallback(() => {
    ...
    const groundPlain = {
      ...
      create: function (scene: THREE.Scene) {
        ...
        // Fix: Use Euler for rotation, not Vector3
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          if (mouseInteraction && this.mouseDistortionStrength > 0) {
            ...
            const mouseRippleNoise = this.simplex(
              // Only pass two arguments to 2D simplex noise; add time to y for animation
              distX_mouse / this.mouseDistortionSmoothness, // Smoothness of the mouse ripple
              (distY_mouse / this.mouseDistortionSmoothness) + this.distortionTime // Animate ripple over time

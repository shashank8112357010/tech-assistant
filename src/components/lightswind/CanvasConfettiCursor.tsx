...
export const CanvasConfettiCursor: React.FC<CanvasConfettiCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const target: EventTarget = fillParent && parentRef.current ? parentRef.current : window;
    const mouseHandler = (event: Event) => moveHandler(event as MouseEvent);
    const touchHandler = (event: Event) => moveHandler(event as TouchEvent);
    target.addEventListener("mousemove", mouseHandler as EventListener);
    target.addEventListener("touchmove", touchHandler as EventListener);
    return () => {
      target.removeEventListener("mousemove", mouseHandler as EventListener);
      target.removeEventListener("touchmove", touchHandler as EventListener);
    };
  }, [fillParent, enabled]);
